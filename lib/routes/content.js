...
const routes = application => {
  return new Promise(resolve => {
     ...
     * Content Home Page
     */
    app.get(`/${config.content.base}`, ensureLoggedIn('/login'), (req, res) => {
     ...
     */
    app.get(`/${config.content.base}/:type`, ensureLoggedIn('/login'), (req, res, next) => {
     ...
     */
    app.get(`/${config.content.base}/:type/${config.content.actions.add}`, ensureLoggedIn('/login'), (req, res, next) => {
      const type = utils.singleItem('id', req.params.type.toLowerCase(), types);
     ...
     */
    app.post(`/${config.content.base}/:type/${config.content.actions.save}`, ensureLoggedIn('/login'), multipartMiddleware, (req, res, next) => {
      const type = utils.singleItem('id', req.params.type.toLowerCase(), types);
