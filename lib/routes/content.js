  ...
  return new Promise(resolve => {
    ...
    */
    app.get(`/${config.content.base}/:type`, (req, res) => {
      return database
        ...
        .select('*')
        .from(`content-type--${req.type.id}`)
        .orderBy('id', 'DESC')
        .orderBy('revision', 'DESC').then(rws => {
          // add itentifier to each row
          const rows = utils.routes.identifier(rws, req.type);
    ...
    */
    app.get(`/${config.content.base}/:type/${config.content.actions.add}`, (req, res, next) => {
      ...
      const steps = _.cloneDeep(req.workflow).steps.reverse();
      const step = steps[steps.length - 1];

      return utils.fill(types, req.type, references, database).then(ct => {
      ...
      return database
        ...
        .orderBy('revision', 'DESC').then(rws => {
          ...
          res.render('content/content', {
            ...
            content: rows,
            type: req.type,
            workflow: _.cloneDeep(req.workflow).steps.reverse(),
            config: config.content,
          });
    ...
    */
    app.post(`/${config.content.base}/:type/${config.content.actions.save}`, multipartMiddleware, (req, res, next) => {
      ...
      // Validation
      const validated = content.form.validate(req.body, req.type, check);
      ...
      if (validated === true) {
        ...
        else if (content.form.validate(req.body, req.type, 'publish') === true) {
          publishable = true;
        }
        ...
        const self = req.workflow.steps[approval].hasOwnProperty('self') && req.workflow.steps[approval].self === true;
        ...
        audits = workflows.audits(rev, req.workflow, request);
