      ...
      // eslint mad if no return, then mad at this else if it is there
        ...
        return utils.fill(req.content.types, req.content.type, references, database).then(ct => {
          ...
          return database(`content-type--${ct.id}`).where({
            ...
            // Add absolute path to files to data
            if (Array.isArray(filers) && filers.length > 0) {
              filers.forEach(filer => {
                // if repeatable
                if (Array.isArray(data[filer.attr])) {
                  data[filer.attr] = data[filer.attr].map(atr => {
                    const attr = atr;
                    if (attr[filer.type].value.hasOwnProperty('original') && attr[filer.type].value.original !== '') {
                      attr[filer.type].value.absolute = path.join(config.storage.public, attr[filer.type].value.relative);
                    }
                  ...
                  // non-repeatables
                  if (data[filer.attr] && data[filer.attr][filer.type].value.hasOwnProperty('original') && data[filer.attr][filer.type].value.original !== '') {
                    data[filer.attr][filer.type].value.absolute = path.join(config.storage.public, data[filer.attr][filer.type].value.relative);
                  }
                }
              });
            }
