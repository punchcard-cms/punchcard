    ...
    */
    app.post(`/${config.content.base}/:type/${config.content.actions.save}`, multipartMiddleware, (req, res, next) => {
      ...
      for (const file in req.files) {
        if (file) {
          body[file] = req.files[file].originalFilename || body[file];
        }
      }

      // Validation
      const validated = types.form.validate(body, req.content.type, check);
      ...
      if (validated === true) {
        ...
        else if (types.form.validate(body, req.content.type, 'publish') === true) {
          publishable = true;
        }
        ...
        // Sunrise/Sunset
        const sunrise = utils.time.iso(body['sunrise-date'], body['sunrise-time'], 'America/New_York');
        const sunset = utils.time.iso(body['sunset-date'], body['sunset-time'], 'America/New_York');
        ...
        if (check === 'publish') {
          if (self) {
            body['comment-textarea'] = 'Self published';
            body['action--select'] = 'self-published';
          }
          else {
            body['comment-textarea'] = 'content submitted';
            body['action--select'] = 'submit';
          }
        }
