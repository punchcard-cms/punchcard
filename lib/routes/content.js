...
const multeropts = {
  storage: multer.diskStorage({
    destination: function (req, file, cb) {
      cb(null, config.storage.temp || 'public/tmp/')
    },
    ...
    */
    app.get(`/${config.content.base}/:type/${config.content.actions.add}`, (req, res, next) => {
      ...
      const step = steps[steps.length - 1];
          ...
          console.log(JSON.stringify(files, null, 2));
      // values existing means the user hit a validation error
      if (files !== {}) {
        ...
        if (Array.isArray(fileinputs) && fileinputs.length > 0 && Array.isArray(files) && files.length > 0) {
          console.log('JSON.stringify(values111, null, 2)');
          ...
          console.log(JSON.stringify(filedata, null, 2));
            ...
            if (filedata) {
              //const filedata = files[`${filer.attr}--${filer.input}`];
