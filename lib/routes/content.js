  ...
  return new Promise(resolve => {
    ...
    */
    app.get(`/${config.content.base}/:type/${config.content.actions.add}`, (req, res, next) => {
      ...
      return utils.fill(types, req.content.type, references, database).then(ct => {
    ...
    */
    app.post(`/${config.content.base}/:type/${config.content.actions.save}`, multipartMiddleware, (req, res, next) => {
      ...
      // Validation
      const validated = content.form.validate(req.body, req.content.type, check);
      ...
      if (validated === true) {
        ...
        else if (content.form.validate(req.body, req.content.type, 'publish') === true) {
          publishable = true;
        }
