      ...
      if (validated === true) {
        ...
        Object.keys(req.body).forEach(key => {
          if ((key === 'sunset-date') || (key === 'sunset-time') || (key === 'sunrise-date') || (key === 'sunrise-time')) {
            ...
            const boom = key.split('--');
            const inputName = boom[0];
            const inputType = boom[1];
            if (boom.length >= 3) {
              const index = boom[2];
              ...
              data[inputName][index][inputType] = { 'value': req.body[key] };
            }
            else {
              if (!data.hasOwnProperty(inputName)) {
                data[inputName] = {};
              }
              data[inputName][inputType] = { 'value': req.body[key] };
            }
          }
        });
