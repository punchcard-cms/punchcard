    ...
    */
    app.post(`/${config.content.base}/:type/${config.content.actions.approve}`, (req, res, next) => {
      ...
      const data = {
        ...
        step: req.body['step--select'],
        author: req.user.id,
        revision,
      };

      return database('workflows').insert(data).then(() => {
        return database(`content-type--${req.params.type.toLowerCase()}`).update('approval', req.body['step--select']).then(() => {
