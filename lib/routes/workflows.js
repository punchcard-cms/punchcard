              ...
              return content.only('workflow', values, model).then(merged => {
                return content.form(merged, errors).then(form => {
                  res.render('content/approval', {
                    ...
                    form,
                    action: `/${config.content.base}/${req.type.id}/${config.content.actions.approve}`,
                    revision,
                    data,
                    type: ct,
      ...
      return workflows.model().then(model => {
        return content.only('workflow', {}, model).then(merged => {
          ...
          if (validated === true) {
            ...
            return database(`content-type--${req.type.id}`).select('*').where('revision', revision).then(rows => {
              const rev = rows[0];
              const audits = workflows.audits(rev, req.workflow, req);

              return database(`content-type--${req.type.id}`)
