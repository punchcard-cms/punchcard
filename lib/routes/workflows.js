    ...
    app.get(`/${config.content.base}/:type/:id/:revision/${config.content.actions.approve}`, (req, res, next) => {
      ...
      return database(`content-type--${type.id}`)
        ...
        .then(rows => {
          ...
          const values = Object.keys(revision.value).map(key => {
            const attr = key.split('--');

            // do not include sunrise/sunset in value
            const suns = ['sunrise-date', 'sunrise-time', 'sunset-date', 'sunset-time'];
            if (suns.indexOf(attr[0]) !== -1) {
              return;
            }

            let value;
            if (Array.isArray(revision.value[key])) {
              value = revision.value[key].map(inputs => {
              ...
              Object.keys(revision.value[key]).forEach(input => {
                value = {
                  value: revision.value[key][input].value,
                  input,
                };
              });
            }
            if (value) {
              // returning the value for .map, should not be bad lint
          ...
          return workflows().then(workflow => {
