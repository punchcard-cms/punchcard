  ...
  if (Array.isArray(files) && files.length > 0) {
    files.forEach(filer => {
      ...
      if (Array.isArray(content[filer.attr])) {
        content[filer.attr] = content[filer.attr].map(atr => {
          const attr = atr;
        ...
        // non-repeatables
        if (content[filer.attr] && content[filer.attr][filer.type].value.hasOwnProperty('original') && content[filer.attr][filer.type].value.original !== '') {
          content[filer.attr][filer.type].value.absolute = path.join(config.storage.public, content[filer.attr][filer.type].value.relative);
        }
      }
    });
  }
