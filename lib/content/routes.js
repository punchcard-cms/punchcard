 ...
 * @param {object} allTypes array of all merged content types
 *
 * @returns {promise} content types form promise returned
 */
const add = (req, res, type, workflow, references, allTypes) => {
  ...
  return utils.fill(allTypes, type, references, database).then(ct => {
    return types.only(type.id, values, [ct]).then(merged => {
