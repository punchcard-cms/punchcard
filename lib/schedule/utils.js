  ...
  return database('live').select('id', 'sunset').where({
    ...
    if (sunset && sunset <= now) {
      return database('live').where({
        ...
        return applications.send(options);
      });
    }
    ...
    if (sunrise <= now) {
      ...
      return database('live').select('id').where({
        ...
        if (items.length === 0) {
          ...
          return database('live').insert(result).then(() => {
            return applications.send(options);
          });
        }
