...
const save = (req, res) => {
  ...
  if (validated === true) {
    ...
    data['client-id'] = 'i must be ruled out';
    data['client-secret'] = 'i must be ruled out';
    console.log('before database promise');

    return database('applications').insert(data).returning('id').then((newid) => {
      _.unset(req.session, 'form.applications.save');
      console.log('database(applications).insert(data).returning(id).then((newid) ');
      res.redirect(`/${config.applications.base}/${newid}`);
