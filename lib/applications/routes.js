 ...
 *
 * @returns {promise} promise wrapping database promise
 */
  ...
  return new Promise((resolve, reject) => {
    return database
      ...
      .orderBy('name', 'DESC').then(rws => {
        ...
        const applications = rows.map(rw => {
          return appsutils.endpoints(rw);
        });
...
const one = (req, res, next) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    return database(`${config.applications.base}`).where({
      ...
      app = appsutils.endpoints(rows[0]);
...
const save = (req, res) => {
  ...
  if (req.body.submit === config.applications.actions.delete && id) {
    return database(`${config.applications.base}`).where('id', '=', id).del().then(() => {
      _.unset(req.session, 'form.applications.edit');
      _.unset(req.session, 'form.applications.save');
  ...
  if (validated === true) {
    ...
    return database(`${config.applications.base}`).insert(data).returning('id').then((newid) => {
      _.unset(req.session, 'form.applications.save');

      return res.redirect(`/${config.applications.base}/${newid}`);
    });
  }
