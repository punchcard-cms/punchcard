...
const save = (req, res) => {
  ...
  console.log('IN SAVE::validated');

  console.log(validated);

  if (validated === true) {
    console.log('IN SAVE::val===true');
    const data = {
    ...
    if (req.body.submit === config.users.actions.update && id) {
      console.log('IN SAVE::update');

      return database('users').where('id', '=', id).update(data).returning('*').then(() => {
        console.log('IN SAVE::update after DB call');
        _.unset(req.session, 'form.users.edit');
        _.unset(req.session, 'form.users.save');
    ...
    return database('users').insert(data).returning('*').then(() => {
      _.unset(req.session, 'form.users.save');
