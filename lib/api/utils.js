...
const reference = (val, input, location, references) => {
  ...
  if (input.hasOwnProperty('type') && input.type === 'reference') {
    ...
    if (type !== null) {
      if (!references.hasOwnProperty(value)) {
        references[value] = {
          paths: [],
          type,
        };
      ...
      references[value].paths.push(location);
  ...
  model.forEach(attr => {
    ...
    if (!attr.hasOwnProperty('repeatable')) {
      ...
      if (total === 1) {
        ...
        const ref = reference(value, attr.inputs[inputs[0]], `${attr.id}.${inputs[0]}.value`, references);
        ...
        inputs.forEach(inp => {
          ...
          // add reference object, if there is one
          const ref = reference(value, attr.inputs[inp], `${attr.id}.${inp}.value`, references);
      ...
      if (_.has(attrs, attr.id)) {
        ...
        result[attr.id] = items.map((item, index) => {
          if (Object.keys(items[0]).length === 1) {
            ...
            // add reference object, if there is one
            const ref = reference(value, attr.inputs[inputs[0]], `${attr.id}[${index}].${inputs[0]}.value`, references);
          ...
          inputs.forEach((inp) => {
            ...
            // add reference object, if there is one
            const ref = reference(value, attr.inputs[inp], `${attr.id}[${index}].${inp}.value`, references);
  ...
  if (Object.keys(references).length > 0) {
    return database.transaction(trx => {
      ...
      return Promise.all(items).then(row => {
        ...
        if (row[0].hasOwnProperty['id']) {
          references[row[0].id].paths.forEach(path => {
...
const one = (query, id, model, models, trx) => {
  ...
  return new Promise(res => {
    return db('live').select('*').where({
      ...
      return format(rows, model, models, query)[0].then(formatted => {
        if (formatted) {
          const type = {
            name: formatted.type,
