...
const attributes = (attrs, model, models, query) => {
  ...
  if (Object.keys(references).length > 0) {
    return database.transaction(trx => {
      // reduce depth after attributes have been gathered at this depth
      // eslint turned off because using isNan causes a knex timeout locally
      if (qry.hasOwnProperty('depth') && parseInt(qry.depth, 10) !== NaN) { // eslint-disable-line use-isnan
        qry.depth--;
      }
      ...
      const items = Object.keys(references).map(id => {
        ...
        return one(qry, id, mod.attributes, models, trx); // eslint-disable-line no-use-before-define
      });

      return Promise.all(items).then(row => {
        for (const val in references) {
          if (references.hasOwnProperty(val)) {
            const paths = references[val].paths;
            paths.forEach(path => {
              _.set(result, path, row[0]);
            });
          }
        }
...
const format = (content, model, models, query) => {
  ...
  const formatted = content.map(item => {
    ...
    if (Object.keys(mod).length < 1 && mods.length > 0) {
      const type = models.find(typ => {
        return typ.id === item['type-slug'];
      });
      mod = type.attributes;
    }

    if (!qry.hasOwnProperty('depth') || isNaN(parseInt(qry.depth, 10))) {
      qry.depth = 0;
    }

    if (Array.isArray(mod) && mod.length > 0 && qry.depth > 0) {
      return attributes(item.attributes, mod, models, qry).then(attrs => {
