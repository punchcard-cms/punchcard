...
const attributes = (attrs, model, models, query) => {
  ...
  model.forEach(attr => {
    if (!attr.hasOwnProperty('repeatable')) {
      ...
      let inputs;
      // content-types adds an entry according the the `min` config attribute - `min` inputs will be added
      ...
      if (_.has(attrs, attr.id)) {
        ...
        result[attr.id] = items.map((item, index) => {
          if (Object.keys(items[0]).length === 1) {
            ...
            // add reference object, if there is one
            const ref = reference(value, attr.inputs[0][inputs[0]], `${attr.id}[${index}].${inputs[0]}`, references);
  ...
  if (Object.keys(references).length > 0) {
    ...
    return database.transaction(trx => {
      ...
      return Promise.all(items).then(row => {
        row.forEach(rw => {
          if (Array.isArray(references[rw.id].paths)) {
...
const format = (content, model, models, query) => {
  ...
  const formatted = content.map(item => {
    ...
    if (Array.isArray(mod) && mod.length > 0 && params.depth >= 0) {
      return attributes(item.attributes, mod, models, params).then(attrs => {
...
const one = (query, id, model, models, trx) => {
  ...
  return db('live').select('*').where({
    ...
    return format(rows, model, models, params).then(fmtd => {
