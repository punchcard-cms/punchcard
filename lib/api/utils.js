 ...
 */
const format = (content, attrs, models, query) => {
  const params = _.cloneDeep(query) || {};
  let atrs = attrs || [];
  const mods = models || [];
  ...
  const formatted = content.map(item => {
    ...
    // we need to get the attributes for each piece of content
    if (mods.length > 1) {
    ...
    if (atrs.length > 0 && params.depth > 0) {
      return attributes(item.attributes, atrs, mods, params).then(attrs => {
