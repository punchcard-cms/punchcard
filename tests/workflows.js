...
test('Workflows compiled into content type', t => {
  return content.raw().then(types => {
    t.is(types[0].workflow, 'default', 'Should have a workflow attribute');
  });
});

test('Workflows grabbed from yaml config', t => {
  return workflows.raw().then(flow => {
    t.is(flow[0].name, 'Self Publish', 'Default should be self-publish');
    t.is(flow[0].id, 'self-publish', 'Default should be self-publish');
    t.true(Array.isArray(flow[0].steps), 'Steps is an array');
    t.is(flow[1].name, 'Editor Approve', 'Second workflow should exist');
    t.is(flow[1].id, 'editor-approve', 'Second workflow should exist');
